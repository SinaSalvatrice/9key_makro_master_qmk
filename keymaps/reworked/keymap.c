...
// Patch: Set all LEDs to a static color for debugging
static void render_rgb_layer_visuals(void) {
    for (uint8_t i = 0; i < PAD_KEY_COUNT; i++) {
        uint8_t led = key_led_map[i];
        if (led < RGBLIGHT_LED_COUNT) {
            rgblight_sethsv_at(128, 255, 96, led); // Hue 128, Sat 255, Val 96 (cyan)
        }
    }
}
#endif
