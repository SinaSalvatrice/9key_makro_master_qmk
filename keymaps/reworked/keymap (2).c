...
/*
 * VIA layer slot mapping.
 *
 * We expose eight configurable palettes to VIA.  These slots map to logical
 * layers in the order specified below.  The order was adjusted to provide
 * RGB control for both the dedicated game layer and the Git (work) layer.
 *
 * Slot 0 -> _BASE   : General navigation / fallback
 * Slot 1 -> _WINDOW : Window and browser controls
 * Slot 2 -> _TEXT   : Text navigation / editing
 * Slot 3 -> _MEDIA  : Media controls
 * Slot 4 -> _DEV    : Game layer (canonical RGB maps _GAME to _DEV)
 * Slot 5 -> _VSC    : Visual Studio Code cockpit
 * Slot 6 -> _RGB    : RGB zone controls (also covers _RGBMOD/_RGBADJ)
 * Slot 7 -> _WORK   : Git/Work layer
 */
static const uint8_t via_layer_slots[VIA_LAYER_SLOT_COUNT] = {
    ...
    _TEXT,    // 2
    _MEDIA,   // 3
    _DEV,     // 4 (Game layer; _GAME canonicalized to _DEV)
    _VSC,     // 5
    _RGB,     // 6
    _WORK     // 7 (Git layer)
};

// Default HSV colours for each VIA palette slot.  These values determine the
// starting hue, saturation and brightness when a layer first activates.  See
// via_layer_slots above for the meaning of each index.
static const hsv_config_t via_default_palette[VIA_LAYER_SLOT_COUNT] = {
    {128, 220, 108}, // BASE
    {176, 240, 112}, // WINDOW
    { 90, 230, 112}, // TEXT
    { 18, 255, 118}, // MEDIA
    { 32, 255, 118}, // GAME (old DEV slot)
    {166, 255, 118}, // VSC
    {210, 255, 124}, // RGB
    { 86, 220, 120}, // GIT/WORK
};
